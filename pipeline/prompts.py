...
# ---------------------------------------------------------------------------
# 阶段 3：翻译原文 + 关键内容标注
# 翻译分段做（避免长文输出截断），标注在完整中文译文上做
# ---------------------------------------------------------------------------
TRANSLATE = """把下面这段文章内容翻译成简体中文。要求：完整翻译，不省略不概括，保留段落结构，专业术语准确，不加译注。如果内容已是中文，原样返回。
...
ANNOTATE = """你是面向企业高管的 AI 组织变革情报分析师。下面是一篇文章的完整中文译文。请标注关键内容。
...
quote 必须是从下面中文译文里逐字截取的原话（与译文文本完全一致，用于高亮定位），长度 15-80 字，能独立理解。note 用简体中文说明。
...
{{
  "annotations": [
    {{"type": "data", "quote": "译文里的原话", "note": "中文说明这条数据为何重要"}},
    {{"type": "decision", "quote": "译文里的原话", "note": "中文说明这个决策的含义"}},
    {{"type": "insight", "quote": "译文里的原话", "note": "中文说明这个观点的启示"}}
  ]
}}"""


def translate_prompt(chunk: str) -> str:
    return TRANSLATE.format(chunk=chunk)
